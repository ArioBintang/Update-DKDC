...
export default function DataJanji() {
  const [isCollapsed, setIsCollapsed] = useState(false);
  const [search, setSearch] = useState("");
  const [janji_temuList, setjanji_temuList] = useState([]);
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setjanji_temuList(Array.isArray(res) ? res : []);
      } catch (err) {
  ...
  const filtered = janji_temuList.filter((p) =>
    p.id_pasien?.toLowerCase().includes(search.toLowerCase())
  );

  const handleEdit = (id) => {
  console.log("Edit janji temu dengan ID:", id);
...
const handleDelete = async (id) => {
  ...
  if (confirm) {
    try {
      await deletejanji(id); // pastikan ada function deletePasien di _services/pasien
      setjanji_temuList(janji_temuList.filter(p => p.id !== id)); // update UI setelah delete
  ...
  return (
    ...
    >
        ...
        <div className="bg-[#b49b50] text-white p-6 rounded-xl shadow-md mb-6">
          <h1 className="text-2xl font-bold">Data Janji Temu</h1>
          ...
          <table className="w-full text-sm">
            <thead className="bg-[#e8dfc8] text-[#3f3f3f]">
              <tr>
                ...
                <th className="p-3">Waktu Janji</th>
  ...
  {filtered.length > 0 ? (
    filtered.map((item, index) => (
      <tr key={item.id} className="border-b hover:bg-[#faf7ee] transition">
        ...
        {/* Kolom lain */}
        <td className="p-3 text-center">{item.id_pasien}</td>
        <td className="p-3 text-center">{item.id_dokter}</td>
        <td className="p-3 text-center">{item.tanggal_janji}</td>
        <td className="p-3 text-center">{item.waktu_janji}</td>
        <td className="p-3 text-center">{item.keluhan}</td>
